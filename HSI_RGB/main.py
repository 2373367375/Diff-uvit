...
def train(max_epoch, batchsz, lr):
    ...
    train_data = Dataset(root+"T1_RGB.mat", root+"T2_HSI.mat", root+"REF.mat", data='Data_4', mode='train', channel=1)
    train_dataloader = DataLoader(train_data, batch_size=32, shuffle=True, num_workers=4, pin_memory=True)
    model = CD_Net().to(device)
    load = False
    if load ==True:
        checkpoint = torch.load('../CD_model/Data_4/best_150.mdl')
    ...
    for epoch in range(max_epoch):
        ...
        if epoch % 10 == 0:
            ...
            torch.save(state, "../CD_model/Data_4/best_{:d}.mdl".format(epoch))
        if train_loss <= best_loss:
            state = dict(epoch=epoch + 1, state_dict=model.state_dict(), best_val=train_loss)
            torch.save(state, '../CD_model/Data_4/best.mdl')
...
def test(model):
    model.eval()
    checkpoint = torch.load('../CD_model/Data_4/best.mdl')
    print("min_loss:", checkpoint['best_val'])
    model.load_state_dict(checkpoint['state_dict'])
    test_data = Dataset(root+"T1_RGB.mat", root+"T2_HSI.mat", root+"REF.mat", data='Data_4', mode='test',
                        channel=1)
    test_dataloader = DataLoader(test_data, batch_size=128, shuffle=False, num_workers=4, pin_memory=True)
    if not os.path.exists('result'):
        os.makedirs('result')
    GT = loadmat('../data/data_4_HSI_RGB/REF.mat')['gt'][0:27*27, 0:27*13]
    H,W = GT.shape
    ...
    with torch.no_grad():
        ...
        filename = "../result/Data_4_result/result.mat"
