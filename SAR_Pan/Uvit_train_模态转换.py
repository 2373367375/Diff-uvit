...
parser.add_argument('--data', default='Data_2')
parser.add_argument('--data_path', default='../data/data_2_SAR_PAN/')
parser.add_argument('--save_path', default='../TS_model/Data_2_SAR_Pan/feature_5/')
parser.add_argument('--load_path', default='../TS_model/Data_2_SAR_Pan/feature_5/model_epoch-1979.pt')
        ...
        for i in range(epoch):
            ...
            if (i + 1) % verbose == 0:
                ...
                for step, [RGB, SAR] in enumerate(tqdm(self.testloader)):
                    ...
                    plt.savefig('../img/Data_2/feature_5/Result_test_' + str(step) + '.jpg')
                    plt.show()
                    plt.close()
...
if __name__ == "__main__":
    ...
    train_data = Dataset(root+"Pan_Norm.mat", root+"SAR_Norm.mat", root+"GT.mat", data=args.data, mode='train', channel=1)
    train_dataloader = DataLoader(train_data, batch_size=batch_size, shuffle=True, num_workers=4, pin_memory=True)
    test_data = Dataset(root+"Pan_Norm.mat", root+"SAR_Norm.mat", root+"GT.mat",data=args.data, mode='test', channel=1)
