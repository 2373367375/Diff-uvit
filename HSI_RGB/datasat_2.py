...
class Dataset(nn.Module):
    ...
    def __init__(self, name1, name2, REF, data, mode, channel=128, padding = 13, nums = 500):
        ...
        self.unpsample = nn.Upsample(scale_factor=3)

        if data == "Data_4":
            self.image_T1 = loadmat(os.path.join(self.filename_T1))['T1']
            # self.image_T1 = self.image_T1.reshape(row * col, -1).transpose(1,0)
            self.image_T2 = self.unpsample(torch.from_numpy(loadmat(os.path.join(self.filename_T2))['T2']).permute(2,0,1).unsqueeze(0))
            self.image_T2 = self.image_T2.squeeze(0).permute(1,2,0).numpy()
        ...
        if self.mode == "train":
            ...
            self.NChanged_point = random.sample(list(np.where(self.whole_point[0] == 2)[0]), int(len(list(np.where(self.whole_point[0] == 2)[0]))*0.1))
    ...
    def __getitem__(self, index):
        ...
        new_j - self.padding: new_j + self.padding+1,:].transpose(2, 0, 1).reshape(3,
                                                                                      self.padding * 2+1,
                                                                                      self.padding * 2+1)

        HSI = self.padding_image_T2[new_i - self.padding: new_i + self.padding+1,
        new_j - self.padding: new_j + self.padding+1,:].transpose(2, 0, 1).reshape(194,
                                                                                      self.padding * 2+1,
                                                                                      self.padding * 2+1)

        GT = self.image_REF[original_i, original_j]-1
