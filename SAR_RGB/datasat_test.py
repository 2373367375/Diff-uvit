...
class Dataset(nn.Module):
    ...
    def __init__(self, name1, name2, REF, data, mode, channel=128, padding = 24, nums = 500):
    ...
    def __getitem__(self, index):
        ...
        RGB = self.padding_image_T1[new_i - self.padding: new_i + self.padding,
        new_j - self.padding: new_j + self.padding, :].transpose(2, 0, 1).reshape(self.channel,
                                                                                      self.padding * 2,
                                                                                      self.padding * 2)

        SAR = self.padding_image_T2[new_i - self.padding: new_i + self.padding,
        new_j - self.padding: new_j + self.padding, :].transpose(2, 0, 1).reshape(self.channel,
                                                                                      self.padding * 2,
                                                                                      self.padding * 2)
