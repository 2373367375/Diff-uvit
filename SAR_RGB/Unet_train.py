...
class UNet(nn.Module):
    ...
    def forward(self, x, timesteps, mode = None):
...
class SR3():
    def __init__(self, device, img_size, LR_size, loss_type, dataloader, testloader,
        ...
        model_RGB_to_SAR = UNet(6, 3)
        model_SAR_to_RGB = UNet(6, 3)
    ...
    def train(self, epoch, verbose):
        ...
        for i in range(epoch):
            ...
            if (i + 1) % verbose == 0:
                ...
                plt.savefig('./img/Unet/Result_train' + str(i) + '.jpg')
                plt.show()
                plt.close()
...
if __name__ == "__main__":
    ...
    sr3 = SR3(device, img_size=img_size, LR_size=LR_size, loss_type='l1',
              ...
              save_path='./model/Unet/',
              load_path='./model/Unet/model_epoch-164.pt', load=True,
              inner_channel=64,
              norm_groups=16, channel_mults=(1, 2, 2, 2), dropout=0, res_blocks=2, lr=1e-4, distributed=True)
    sr3.train(epoch=10000, verbose=50)
