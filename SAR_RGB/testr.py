...
class Dataset(nn.Module):
    ...
    def __init__(self, name1, name2, REF, data, mode, channel=128, padding = 13, nums = 500):
        ...
        self.filename_T2_Mat = '/media/xidian/55bc9b72-e29e-4dfa-b83e-0fbd0d5a7677/xd132/HJ/change_detection/赵师姐AAAI/mat文件/'
        if data == "Data_1":
            self.image_RGB_org = loadmat(os.path.join(self.filename_T1))['RGB_Norm']
            [row, col, bands] = self.image_T1.shape
            self.image_SAR_org = loadmat(os.path.join(self.filename_T2))['SAR_Norm']

            self.image_RGB = loadmat(os.path.join(self.filename_T2_Mat+'RGB.mat'))['RGB']
            self.image_SAR = loadmat(os.path.join(self.filename_T2_Mat+'SAR.mat'))['RGB']
        ...
        self.padding_image_T1_org = cv.copyMakeBorder(self.image_RGB_org, self.padding, self.padding, self.padding, self.padding, cv.BORDER_REFLECT)
        self.padding_image_T2_org = cv.copyMakeBorder(self.image_SAR_org, self.padding, self.padding, self.padding, self.padding, cv.BORDER_REFLECT)
        self.padding_image_T1 = cv.copyMakeBorder(self.image_RGB, self.padding, self.padding, self.padding,
                                                      self.padding, cv.BORDER_REFLECT)
        self.padding_image_T2 = cv.copyMakeBorder(self.image_SAR, self.padding, self.padding, self.padding,
                                                      self.padding, cv.BORDER_REFLECT)
    ...
    def __getitem__(self, index):
        ...
        RGB_org = self.padding_image_T1_org[new_i - self.padding: new_i + self.padding+1,
        new_j - self.padding: new_j + self.padding+1, :].transpose(2, 0, 1).reshape(self.channel,
                                                                                      self.padding * 2+1,
                                                                                      self.padding * 2+1)

        SAR_org = self.padding_image_T2_org[new_i - self.padding: new_i + self.padding+1,
        new_j - self.padding: new_j + self.padding+1, :].transpose(2, 0, 1).reshape(self.channel,
                                                                                      self.padding * 2+1,
                                                                                      self.padding * 2+1)

        RGB = self.padding_image_T2[new_i - self.padding: new_i + self.padding + 1,
              new_j - self.padding: new_j + self.padding + 1, :].transpose(2, 0, 1).reshape(self.channel,
                                                                                            self.padding * 2 + 1,
                                                                                            self.padding * 2 + 1)
