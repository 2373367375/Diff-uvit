...
class UViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=13, in_chans=1, embed_dim=256, depth=4, num_heads=8, mlp_ratio=4.,
        ...
        self.final_layer = nn.Conv2d(self.in_chans, 1, 3, 1,1) if conv else nn.Identity()
...
class Block_CD(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None,
        ...
        self.patch_embed_1 = PatchEmbed(patch_size=patch_size, in_chans=1, embed_dim=256)
        self.patch_embed_2 = PatchEmbed(patch_size=patch_size, in_chans=1, embed_dim=256)
        # self.noise_func = FeatureWiseAffine(self.in_chans, self.in_chans)
...
class CD_Net(nn.Module):
    ...
    def __init__(self, embed_dim=256, num_heads=1, mlp_ratio=4, qkv_bias=False, qk_scale=None, norm_layer=nn.LayerNorm,
        ...
        self.conv_feature = nn.Conv2d(1*4, 1, 3, 1, 1)
    ...
    def forward(self, Data_patch):
        ...
        SAR_step_0 = self.CD_blocks[3](Data_patch['T2_f_step0_0_patch'], Data_patch['T1_x_0_patch'])
        RGB_step_0 = self.CD_blocks[4](Data_patch['T2_x_0_patch'], Data_patch['T1_f_step0_0_patch'])
        SAR_step_1000 = self.CD_blocks[5](Data_patch['T2_f_step1000_0_patch'], Data_patch['T1_f_step1000_6_patch'])
        RGB_step_1000 = self.CD_blocks[6](Data_patch['T2_f_step1000_6_patch'], Data_patch['T1_f_step1000_0_patch'])
        SAR_step_2000 = self.CD_blocks[7](Data_patch['T2_f_step2000_0_patch'], Data_patch['T1_f_step2000_6_patch'])
        RGB_step_2000 = self.CD_blocks[8](Data_patch['T2_f_step2000_6_patch'], Data_patch['T1_f_step2000_0_patch'])
