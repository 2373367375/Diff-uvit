...
parser.add_argument('--device', default='cuda:0', choices=['cuda:0', 'cuda:1'])
parser.add_argument('--data', default='Data_3')
parser.add_argument('--data_path', default='../data/data_3_NIR_RGB/')
parser.add_argument('--save_path', default='../TS_model/Data_3_NIR_RGB/feature_5/')
parser.add_argument('--load_path', default='../TS_model/Data_3_NIR_RGB/feature_5/model_epoch-1979.pt')
...
class Diffusion(nn.Module):
    ...
    def p_mean_variance(self, x, t, clip_denoised: bool, condition_x=None, mode=None):
        ...
        if mode == 'RGB_to_SAR':
            x_start, _ = self.model_RGB_to_SAR(torch.cat([condition_x, x], dim=1), timesteps = noise_level)
        elif mode == 'SAR_to_RGB':
            x_start, _ = self.model_SAR_to_RGB(torch.cat([condition_x, x], dim=1), timesteps = noise_level)
...
class SR3():
    ...
    def train(self, epoch, verbose):
        ...
        for i in range(epoch):
            ...
            if (i + 1) % verbose == 0:
                self.sr3.eval()
                for step, [RGB, SAR, GT] in enumerate(tqdm(self.testloader)):
                    # [RGB, SAR] = test_data
                    RGB = RGB.to(self.device).type(torch.float32)
                    SAR = SAR.to(self.device).type(torch.float32)
                    ...
                    plt.savefig('../img/Data_3/feature_5/Result_test_' + str(step) + '.jpg')
                    plt.show()
                    plt.close()
                    break
...
if __name__ == "__main__":
    ...
    train_data = Dataset(root+"RGB_Norm.mat", root+"NIR_Norm.mat", root+"GT.mat", data=args.data, mode='train', channel=3)
    train_dataloader = DataLoader(train_data, batch_size=batch_size, shuffle=True, num_workers=4, pin_memory=True)
    test_data = Dataset(root+"RGB_Norm.mat", root+"NIR_Norm.mat", root+"GT.mat", data=args.data, mode='test', channel=3)
    ...
    sr3 = SR3(device, img_size=img_size, LR_size=LR_size, loss_type='l1',
              ...
              load_path=args.load_path, load=False,
              inner_channel=64,
