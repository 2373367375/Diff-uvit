...
def train(max_epoch, batchsz, lr):
    ...
    train_data = Dataset(root+"RGB_Norm.mat", root+"NIR_Norm.mat", root+"GT.mat", data='Data_3', mode='train', channel=1)
    ...
    load = False
    if load ==True:
        checkpoint = torch.load('../CD_model/Data_3/best_150.mdl')
    ...
    for epoch in range(max_epoch):
        ...
        if epoch % 25 == 0:
            ...
            torch.save(state, "../CD_model/Data_3/best_{:d}.mdl".format(epoch))
        if train_loss <= best_loss:
            state = dict(epoch=epoch + 1, state_dict=model.state_dict(), best_val=train_loss)
            torch.save(state, '../CD_model/Data_3/best.mdl')
...
def test(model):
    model.eval()
    checkpoint = torch.load('../CD_model/Data_3/best_25.mdl')
    print("min_loss:", checkpoint['best_val'])
    model.load_state_dict(checkpoint['state_dict'])
    test_data = Dataset(root+"RGB_Norm.mat", root+"NIR_Norm.mat", root+"GT.mat", data='Data_3', mode='test',
                        channel=1)
    test_dataloader = DataLoader(test_data, batch_size=128, shuffle=False, num_workers=4, pin_memory=True)
    if not os.path.exists('result'):
        os.makedirs('result')
    GT = loadmat('../data/data_3_NIR_RGB/GT.mat')['GT'][0:297, 0:405, 0]
    H,W = GT.shape
    ...
    with torch.no_grad():
        ...
        filename = "../result/Data_3_result/result.mat"
