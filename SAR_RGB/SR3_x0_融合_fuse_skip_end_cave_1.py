...
import numpy as np
import math, os, copy
from DataLoad_noupsample import *
...
def SAM(ref, tar):
    ...
    for i in range(b):
        ...
        y = y.permute(1, 2, 0).reshape(-1, c)
        A = torch.sum(x * y, dim=-1) / (
                torch.sqrt(torch.sum(x * x, dim=-1)) * torch.sqrt(torch.sum(y * y, dim=-1)))
        _SAM = torch.arccos(A) * 180 / np.pi
        SAM = torch.mean(_SAM) + SAM
    return SAM/b
...
class SelfAtt(nn.Module):
    def __init__(self, channel_dim, num_heads, norm_groups=32, att_num=0.0):
        ...
        for i, layer in enumerate(self.ups):
            ...
            if isinstance(layer, ResBlock):
                if i == 0:
                    ...
                    # x = layer(torch.cat([x, temp_feats_skip, temp_feats], dim=1), t)
                    z = z+1
                else:
                    x = layer(torch.cat([x, feats.pop(), feats_skip.pop()], dim=1), t)
...
#         x_true_hat = x_true[i].permute(1,2,0)
#         x_pred_hat = x_pred[i].permute(1,2,0)
#         assert x_true_hat.ndim == 3 and x_true_hat.shape == x_pred_hat.shape
#         dot_sum = np.sum(x_true_hat * x_pred_hat, axis=2)
#         norm_true = np.linalg.norm(x_true_hat, axis=2)
#         norm_pred = np.linalg.norm(x_pred_hat, axis=2)
#
...
class Diffusion(nn.Module):
    def __init__(self, model_spe, model_spa, device, img_size, LR_size, channels=3):
        ...
        self.fuse = nn.Sequential(
            ...
            # nn.Conv2d(31, 31, kernel_size=1, stride=1, padding=0),
        ).to(device)
    ...
    def net(self, gtHS, hrMS, lrHS):
        ...
        # t = np.random.randint(1, self.num_timesteps + 1)
        # sqrt_alpha = torch.FloatTensor(
        #     np.random.uniform(self.sqrt_alphas_cumprod_prev[t - 1], self.sqrt_alphas_cumprod_prev[t], size=b)).to(gtHS.device)
        # sqrt_alpha = sqrt_alpha.view(-1, 1, 1, 1)
        ...
        loss_sam_fuse = SAM(pred_x0_fuse, gtHS)
...
class SR3():
    def __init__(self, device, img_size, LR_size, loss_type, dataloader, testloader,
        ...
        model_spe = UNet(34, 31, 31, inner_channel, norm_groups, channel_mults, res_blocks, dropout, img_size)
        model_spa = UNet(62, 31, 3, inner_channel, norm_groups, channel_mults, res_blocks, dropout, img_size)
        ...
        for i in range(epoch):
            ...
            if train:
                for step, [gtHS, hrMS, lrHS] in enumerate(tqdm(self.dataloader)):
                    ...
                    # loss = loss_1 + loss_2 + 0*loss_3 + loss_4 + loss_5 + loss_6 + loss_7 + 0*loss_sam
                    if i<=0:
                        loss = loss_1 + loss_2 + loss_4 + loss_5
                    else:
                        loss = loss_1 + loss_2 + 0.1*loss_4 + 0.1*loss_5 + loss_7
                    loss.backward()
                    self.optimizer.step()
                    ...
                    loss_sam_epoch = loss_sam.item() + loss_sam_epoch
                    ...
                    #                                                             nrow=2, padding=1, normalize=True).cpu(),(1, 2, 0))[:,:,[2,1,0]])
                    #
                    #         plt.subplot(1, 4, 2)
                    #         plt.axis("off")
                    #         plt.title("HSI_input")
                    ...
                    #                                                             nrow=2, padding=1, normalize=True), (1, 2, 0))[:,:,[30,15,5]])
                    #
                    #         plt.subplot(1, 4, 3)
                    #         plt.axis("off")
                    #         plt.title("HSI_output")
                    #         # A = self.test(test_img, test_lrHS_img)
                    #         plt.imshow(np.transpose(torchvision.utils.make_grid(self.test(lrHS.unsqueeze(0), gtHS.unsqueeze(0)).cpu(),
                    #                                                             nrow=2, padding=1, normalize=True), (1, 2, 0))[:,:,[30,15,5]])
                    #
                    #         plt.subplot(1, 4, 4)
                    #         plt.axis("off")
                    ...
                    #                                                             nrow=2, padding=1, normalize=True),(1, 2, 0))[:,:,[30,15,5]])
                    #
                    #         plt.savefig('./img/cave/SuperResolution_Result_train'+str(i)+'.jpg')
                    #         plt.show()
                    #         plt.close()
            ...
            if (i + 1) % verbose == 0:
                ...
                plt.savefig('./img/cave_x0_fuse_16CH/SuperResolution_Result_test' + str(i) + '.jpg')
                plt.show()
                plt.close()
...
if __name__ == "__main__":
    ...
    LR_size = 32
    img_size = 128
    root = '/media/xidian/55bc9b72-e29e-4dfa-b83e-0fbd0d5a7677/xd132/HJ/defusion/data/cave_cjz/'
    ...
    sr3 = SR3(device, img_size=img_size, LR_size=LR_size, loss_type='l1',
              ...
              save_path='/media/xidian/55bc9b72-e29e-4dfa-b83e-0fbd0d5a7677/xd132/HJ/defusion/cycle/model/cave_x0_fuse_16CH/',
              load_path='./model/cave_x0_fuse_16CH/SR3_model2_epoch-799.pt', load=False,
              inner_channel=64,
