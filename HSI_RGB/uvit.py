...
class UViT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=13, in_chans=197, out_chans=1, embed_dim=256, depth=4, num_heads=8, mlp_ratio=4.,
        ...
        self.conv1 = nn.Conv2d(embed_dim, self.out_chans, 3, 1, 1)
        self.final_layer = nn.Conv2d(self.out_chans, self.out_chans, 3, 1, 1) if conv else nn.Identity()
    ...
    def forward(self, x, timesteps, y=None):
        feature = []
        x_org = x
        if self.out_chans < 50:
            feature.append(x_org[:, 0:194, :, :])
        else:
...
class Block_CD(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None,
                 act_layer=nn.GELU, norm_layer=nn.LayerNorm, skip=False, use_checkpoint=False, patch_size=1, channel=None):
        super().__init__()
        self.norm1 = norm_layer(dim)
        self.norm2 = norm_layer(dim)
        ...
        self.patch_embed_1 = PatchEmbed(patch_size=patch_size, in_chans=channel, embed_dim=256)
        self.patch_embed_2 = PatchEmbed(patch_size=patch_size, in_chans=channel, embed_dim=256)
        # self.noise_func = FeatureWiseAffine(self.in_chans, self.in_chans)
...
class CD_Net(nn.Module):
    ...
    def __init__(self, embed_dim=256, num_heads=1, mlp_ratio=4, qkv_bias=False, qk_scale=None, norm_layer=nn.LayerNorm,
                 use_checkpoint=None, channel=None):
        super().__init__()

        self.CD_blocks = nn.ModuleList([
            Block_CD(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                     qk_scale=qk_scale,
                     norm_layer=norm_layer, use_checkpoint=use_checkpoint, channel=3)
            for _ in range(6)])

        self.CD_blocks_2 = nn.ModuleList([
            Block_CD(dim=embed_dim, num_heads=num_heads, mlp_ratio=mlp_ratio, qkv_bias=qkv_bias,
                     qk_scale=qk_scale,
                     norm_layer=norm_layer, use_checkpoint=use_checkpoint, channel=194)
            for _ in range(3)])
        ...
        self.conv_feature_2 = nn.Conv2d(194 * 4, 3, 3, 1, 1)
    ...
    def forward(self, Data_patch):
        ...
        feature_step_SAR_0 = self.conv_feature_2(torch.cat(
        ...
        feature_step_SAR_1000 = self.conv_feature_2(torch.cat(
        ...
        feature_step_SAR_2000 = self.conv_feature_2(torch.cat(
        ...
        RGB_step_0 = self.CD_blocks[3](Data_patch['T2_f_step0_0_patch'], Data_patch['T1_x_0_patch'])
        RGB_step_1000 = self.CD_blocks[4](Data_patch['T2_f_step1000_0_patch'], Data_patch['T1_f_step1000_6_patch'])
        RGB_step_2000 = self.CD_blocks[5](Data_patch['T2_f_step2000_0_patch'], Data_patch['T1_f_step2000_6_patch'])
        HSI_step_0 = self.CD_blocks_2[0](Data_patch['T2_x_0_patch'], Data_patch['T1_f_step0_0_patch'])
        HSI_step_1000 = self.CD_blocks_2[1](Data_patch['T2_f_step1000_6_patch'], Data_patch['T1_f_step1000_0_patch'])
        HSI_step_2000 = self.CD_blocks_2[2](Data_patch['T2_f_step2000_6_patch'], Data_patch['T1_f_step2000_0_patch'])
        ...
        HSI_step_0 = self.flatten(HSI_step_0)
        RGB_step_0 = self.flatten(RGB_step_0)
        HSI_step_1000 = self.flatten(HSI_step_1000)
        RGB_step_1000 = self.flatten(RGB_step_1000)
        HSI_step_2000 = self.flatten(HSI_step_2000)
        ...
        HSI_step_0 = self.Classfy2(HSI_step_0)
        HSI_step_1000 = self.Classfy2(HSI_step_1000)
        HSI_step_2000 = self.Classfy2(HSI_step_2000)
        ...
        out = self.w1 * feature_step_0+self.w2* feature_step_1000+self.w3 * feature_step_2000\
              ...
              +self.w7 * HSI_step_0 + self.w8 * HSI_step_1000 + self.w9 * HSI_step_2000
        output = out
