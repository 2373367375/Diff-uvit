...
class Diffusion(nn.Module):
    ...
    def p_mean_variance(self, x, t, clip_denoised: bool, condition_x=None, mode=None):
        ...
        return mean, posterior_log_variance, feature
    ...
    def p_sample(self, img_RGB, img_SAR, t, clip_denoised=True, condition_RGB=None, condition_SAR=None):
        ...
        mean1, log_variance1, feature1 = self.p_mean_variance(x=img_SAR, t=t, clip_denoised=clip_denoised, condition_x=condition_RGB, mode='RGB_to_SAR')

        mean2, log_variance2, feature2 = self.p_mean_variance(x=img_RGB, t=t, clip_denoised=clip_denoised, condition_x=condition_SAR, mode='SAR_to_RGB')

        noise = torch.randn_like(img_RGB) if t > 0 else torch.zeros_like(img_SAR)
        return mean1 + noise * (0.5 * log_variance1).exp(), feature1, mean2 + noise * (0.5 * log_variance2).exp(), feature2
    ...
    def super_resolution(self, RGB, SAR):
        ...
        for i in reversed(range(0, self.num_timesteps)):
            ...
            img_SAR, feature_SAR, img_RGB, feature_RGB = self.p_sample(img_RGB, img_SAR, i, condition_RGB=RGB, condition_SAR=SAR)
            if i>=(2000-num) or i<num:
                feature_SAR_2000.append(feature_SAR)
                feature_RGB_2000.append(feature_RGB)
        return img_SAR, feature_SAR_2000, img_RGB, feature_RGB_2000
...
class SR3():
    ...
    def train(self, epoch, verbose):
        ...
        for i in range(epoch):
            ...
            if (i + 1) % verbose == 0:
                ...
                img_SAR, feature_SAR, img_RGB, feature_RGB = self.test(RGB, SAR)
                ...
                plt.savefig('./img/Modal_transformation/Result_test' + str(i) + '.jpg')
                plt.show()
                plt.close()
                self.optimizer.zero_grad()
                out = self.Net(img_SAR, feature_SAR, img_RGB, feature_RGB)
    ...
    # def CD_Net(self, img_SAR, feature_SAR, img_RGB, feature_RGB):
    #
    #     # RGB = RGB
    #     # SAR = SAR
    #
    ...
    def CD_save(self, save_path, i):
        ...
        for key, param in state_dict.items():
            state_dict[key] = param.cpu()
        torch.save(state_dict, save_path + '/CD_model/' + 'CD_model_epoch-{}.pt'.format(i))
...
if __name__ == "__main__":
    ...
    sr3 = SR3(device, img_size=img_size, LR_size=LR_size, loss_type='l1',
              ...
              save_path='./model/Patch/',
              load_path='./model/Patch/model_epoch-87.pt', load=False,
              inner_channel=64,
