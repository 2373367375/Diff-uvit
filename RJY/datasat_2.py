...
class Dataset(nn.Module):
    ...
    def __init__(self, name1, name2, REF, data, mode, channel=128, padding = 13, nums = 500):
        ...
        if data == "Data_2":
            self.image_T1 = loadmat(os.path.join(self.filename_T1))['LRHSt1_norm'][0:307, 0:241, :]
            # self.image_T1 = self.image_T1.reshape(row * col, -1).transpose(1,0)
            self.image_T2 = loadmat(os.path.join(self.filename_T2))['rgb'][0:307, 0:241, :]
            # self.image_T2 = self.image_T2.reshape(row * col, -1).transpose(1, 0)
            self.image_REF = loadmat(os.path.join(self.REF))['Binary'][0:307, 0:241]
    ...
    def __getitem__(self, index):
        ...
        T1 = self.padding_image_T1[new_i - self.padding: new_i + self.padding+1,
        new_j - self.padding: new_j + self.padding+1].transpose(2, 0, 1).reshape(154,
                                                                                      self.padding * 2+1,
                                                                                      self.padding * 2+1)

        T2 = self.padding_image_T2[new_i - self.padding: new_i + self.padding+1,
