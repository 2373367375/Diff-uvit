...
class Dataset(nn.Module):
    ...
    def __init__(self, name1, name2, REF, data, mode, channel=128, padding = 13, nums = 500):
        ...
        if data == "Data_3":
            self.image_T1 = loadmat(os.path.join(self.filename_T1))['RGB_Norm'][0:297, 0:405, :]
            # self.image_T1 = self.image_T1.reshape(row * col, -1).transpose(1,0)
            self.image_T2 = loadmat(os.path.join(self.filename_T2))['NIR_Norm'][0:297, 0:405, :]
            # self.image_T2 = self.image_T2.reshape(row * col, -1).transpose(1, 0)
            self.image_REF = loadmat(os.path.join(self.REF))['GT'][0:297, 0:405, 0]
