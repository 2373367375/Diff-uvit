...
class Dataset(nn.Module):
    ...
    def __init__(self, name1, name2, REF, data, mode, channel=128, padding = 13, nums = 500):
        ...
        if data == "Data_4":
            self.image_T1 = loadmat(os.path.join(self.filename_T1))['T1'][0:27*27, 0:27*13, :]
            # self.image_T1 = self.image_T1.reshape(row * col, -1).transpose(1,0)
            self.image_T2 = self.unpsample(torch.from_numpy(loadmat(os.path.join(self.filename_T2))['T2']).permute(2,0,1).unsqueeze(0)).squeeze(0).permute(1,2,0)[0:27*27, 0:27*13, :]
            # self.image_T2 = self.image_T2.reshape(row * col, -1).transpose(1, 0)
            self.image_REF = loadmat(os.path.join(self.REF))['gt'][0:27*27, 0:27*13]
    ...
    def __getitem__(self, index):
        ...
        return self.Patch_T1[index].transpose(2,0,1), self.Patch_T2[index].permute(2,0,1)
