...
class Diffusion(nn.Module):
    ...
    def p_mean_variance(self, x, t, clip_denoised: bool, condition_x=None, mode=None):
        ...
        if mode == 'RGB_to_SAR':
            x_start = self.model_RGB_to_SAR(torch.cat([condition_x, x], dim=1), timesteps = noise_level)
        elif mode == 'SAR_to_RGB':
            x_start = self.model_SAR_to_RGB(torch.cat([condition_x, x], dim=1), timesteps = noise_level)
    ...
    def p_sample(self, img_RGB, img_SAR, t, clip_denoised=True, condition_RGB=None, condition_SAR=None):
        ...
        mean1, log_variance1 = self.p_mean_variance(x=img_RGB, t=t, clip_denoised=clip_denoised, condition_x=condition_RGB, mode='RGB_to_SAR')

        mean2, log_variance2 = self.p_mean_variance(x=img_SAR, t=t, clip_denoised=clip_denoised, condition_x=condition_SAR, mode='SAR_to_RGB')
    ...
    def super_resolution(self, RGB, SAR):
        ...
        for i in reversed(range(0, self.num_timesteps)):
            ...
            img_RGB, img_SAR = self.p_sample(img_RGB, img_SAR, i, condition_RGB=RGB, condition_SAR=SAR)
    ...
    def net(self, RGB, SAR):
        ...
        # The model predict actual noise added at time step t
        RGB_to_SAR_x0 = self.model_RGB_to_SAR(torch.cat([RGB, x_noisy_RGB], dim=1), timesteps=sqrt_alpha)
        SAR_to_RGB_x0 = self.model_SAR_to_RGB(torch.cat([SAR, x_noisy_SAR], dim=1), timesteps=sqrt_alpha)

        loss_1 = self.loss_func(RGB, RGB_to_SAR_x0) / int(b * c * h * w)
        loss_2 = self.loss_func(SAR, SAR_to_RGB_x0) / int(b * c * h * w)
...
class SR3():
    def __init__(self, device, img_size, LR_size, loss_type, dataloader, testloader,
        ...
        model_RGB_to_SAR = UViT(32, 1, 6)
        model_SAR_to_RGB = UViT(32, 1, 6)
    ...
    def train(self, epoch, verbose):
        ...
        for i in range(epoch):
            ...
            if (i + 1) % verbose == 0:
                ...
                img_SAR, img_RGB = self.test(RGB.unsqueeze(0), SAR.unsqueeze(0))
                ...
                plt.savefig('./img/Rebuild/Result_train' + str(i) + '.jpg')
                plt.show()
                plt.close()
...
if __name__ == "__main__":
    ...
    sr3 = SR3(device, img_size=img_size, LR_size=LR_size, loss_type='l1',
              ...
              save_path='./model/Rebuild/',
              load_path='./model/Rebuild/model_epoch-19.pt', load=False,
              inner_channel=64,
              norm_groups=16, channel_mults=(1, 2, 2, 2), dropout=0, res_blocks=2, lr=1e-4, distributed=False)
    sr3.train(epoch=10000, verbose=10)
